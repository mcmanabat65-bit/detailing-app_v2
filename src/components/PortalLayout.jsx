...
const links = [
  ...
  { href: '/portal/book', label: 'Book a Detail', icon: CalendarPlus },
  { href: '/portal/bookings', label: 'My Bookings', icon: ClipboardList },
  { href: '/portal/fleet', label: 'My Fleet', icon: Car },
  { href: '/portal/profile', label: 'Profile', icon: UserCog },
];

export function PortalLayout({ children, title }) {
  const { signOut, showToast, currentMember } = useApp();
    ...
    <div className="min-h-screen bg-obsidian flex">
      ...
      <aside className="hidden md:flex w-64 shrink-0 flex-col border-r border-white/5 bg-surface/30 sticky top-0 h-screen overflow-y-auto">
        ...
        <div className="px-6 py-3 border-b border-white/5">
          <div className="text-cream text-sm font-medium truncate">{firstName}</div>
          <span className="inline-flex items-center gap-1.5 mt-1 text-[10px] uppercase tracking-widest text-gold bg-gold/10 border border-gold/30 rounded-full px-2.5 py-1">
            <Crown className="w-3 h-3" />
            VIP Member
          </span>
        </div>

        <nav className="flex-1 px-3 py-6 space-y-1">{nav()}</nav>
      ...
      >
        <div className="flex items-center justify-between px-4 h-16 border-b border-white/5">
          <div className="font-serif text-cream">Member Portal</div>
          <button
            onClick={() => setOpen(false)}
